...
class Args:
    ...
    # WebShop Critic Error supervision
    critic_error_enabled: bool = False
    """Enable dynamic Critic Error weakness mining, rubric injection, and pool retirement."""
    critic_error_pool_path: Optional[str] = None
    """JSONL path for active Critic Error weaknesses."""
    critic_error_update_every_steps: int = 256
    """Mine new Critic Error weaknesses every N training steps."""
    critic_error_inject_every_steps: int = 128
    """Inject active Critic Error rubrics into reward ground truth every N training steps."""
    critic_error_retire_after_absent_batches: int = 3
    """Retire a weakness after this many judged batches without triggering."""
    critic_error_judge_model: str = "kimi-k2.6"
    """OpenAI-compatible judge/supervisor model used for Critic Error mining and scoring."""
    critic_error_openai_base_url: str = "https://ark.cn-beijing.volces.com/api/plan/v3"
    """OpenAI-compatible base URL for the Critic Error judge/supervisor."""
    critic_error_api_key_env: str = "SEED_AGENT_PLAN_API_KEY"
    """Environment variable that stores the Critic Error judge API key."""
    critic_error_max_active_weaknesses: int = 8
    """Maximum active weaknesses to inject into a WebShop reward call."""
    critic_error_trigger_threshold: float = 0.55
    """Judge score threshold used to decide whether a weakness is still triggered."""
    webshop_base_url: str = "http://127.0.0.1:36001"
    """WebShop FastAPI base URL used to replay generated action trajectories."""
    webshop_replay_enabled: bool = True
    """Replay extracted WebShop actions against the environment for task reward."""
    webshop_replay_timeout: float = 15.0
    """HTTP timeout for each WebShop replay request."""
    webshop_max_replay_steps: int = 8
    """Maximum number of generated WebShop actions to replay."""
    webshop_interactive_rollout: bool = False
    """Generate WebShop rollouts by alternating one model action with one environment step."""
    webshop_action_max_tokens: int = 80
    """Maximum tokens to sample for one WebShop action turn during interactive rollout."""
    webshop_observation_token_budget: int = 384
    """Maximum masked observation tokens appended after each WebShop environment step."""
    webshop_rollout_trace_path: Optional[str] = None
    """Optional JSONL path to save WebShop rollout traces before GRPO filtering."""
...
WEBSHOP_ACTION_RE = re.compile(r"\b(search|click)\[([^\]\n]+)\]", re.IGNORECASE)


def parse_webshop_ground_truth(ground_truth: Any) -> Dict[str, Any]:
...
def extract_last_webshop_action(text: str) -> Optional[str]:
    matches = list(WEBSHOP_ACTION_RE.finditer(text))
    if not matches:
        return None
    match = matches[-1]
...
def webshop_action_prefix(available_actions: Optional[Dict[str, Any]]) -> str:
    if isinstance(available_actions, dict) and available_actions.get("has_search_bar"):
        return "\nAction: search["
    return "\nAction: click["


def encode_masked_webshop_observation(
    tokenizer: PreTrainedTokenizer,
    text: str,
    token_budget: int,
) -> List[int]:
...
def create_webshop_env(base_url: str, session_id: Optional[int], timeout: float) -> int:
...
def webshop_available_actions(base_url: str, env_idx: int, timeout: float) -> Dict[str, Any]:
    base = base_url.rstrip("/")
    resp = requests.get(f"{base}/available_actions", params={"env_idx": env_idx}, timeout=timeout)
...
def format_webshop_observation_block(
    state: str,
...
def vllm_generate_thread(
    ...
    eval_ground_truths: Optional[List[Any]],
    ...
    def generate_webshop_rollouts_with_engines(
        ...
        step_sampling_params.n = 1
        step_sampling_params.stop = ["</s>", "]"]
        step_sampling_params.include_stop_str_in_output = True
        step_sampling_params.max_tokens = min(args.webshop_action_max_tokens, sampling_params.max_tokens)
        rollouts = []
        base = args.webshop_base_url.rstrip("/")
        for prompt, ground_truth in zip(prompts, ground_truths):
            gt = parse_webshop_ground_truth(ground_truth)
            session_id = gt.get("webshop_session_id")
            initial_available_actions = gt.get("initial_available_actions") or {"has_search_bar": True}
            for _ in range(samples_per_prompt):
                rollout = {
                    ...
                    "num_calls": 0,
                    "last_reward": 0.0,
                    "next_action_prefix": webshop_action_prefix(initial_available_actions),
                }
                try:
                    rollout["env_idx"] = create_webshop_env(base, session_id, args.webshop_replay_timeout)
                    available = webshop_available_actions(base, int(rollout["env_idx"]), args.webshop_replay_timeout)
                    if available:
                        rollout["next_action_prefix"] = webshop_action_prefix(available)
                except Exception as exc:
                    rollout["done"] = True
                    rollout["finish_reason"] = "env_error"
                    rollout["error"] = f"{type(exc).__name__}: {str(exc)[:200]}"
                rollouts.append(rollout)

        for _ in range(args.webshop_max_replay_steps):
            active_indices = [
                i
                for i, rollout in enumerate(rollouts)
                if not rollout["done"] and len(rollout["response"]) < sampling_params.max_tokens
            ]
            if not active_indices:
                break

            step_prompts = []
            prompt_rollout_indices = []
            step_max_tokens = args.webshop_action_max_tokens
            for i in active_indices:
                rollout = rollouts[i]
                remaining = sampling_params.max_tokens - len(rollout["response"])
                if remaining <= 0:
                    rollout["done"] = True
                    rollout["finish_reason"] = "length"
                    continue
                prefix_ids = tokenizer.encode(str(rollout.get("next_action_prefix") or "\nAction: "), add_special_tokens=False)
                if remaining <= len(prefix_ids):
                    rollout["done"] = True
                    rollout["finish_reason"] = "length"
                    continue
                rollout["response"].extend(prefix_ids)
                rollout["mask"].extend([0] * len(prefix_ids))
                remaining -= len(prefix_ids)
                step_max_tokens = min(step_max_tokens, remaining)
                step_prompts.append(rollout["prompt"] + rollout["response"])
                prompt_rollout_indices.append(i)

            if not step_prompts:
                break
            step_sampling_params.max_tokens = max(1, step_max_tokens)
            step_response_ids, step_finish_reasons, _, _ = generate_with_engines(step_prompts, step_sampling_params)

            for rollout_idx, response_tokens, step_finish_reason in zip(
                prompt_rollout_indices, step_response_ids, step_finish_reasons
            ):
                ...
                rollout["response"].extend(response_tokens)
                rollout["mask"].extend([1] * len(response_tokens))

                decoded_so_far = tokenizer.decode(rollout["response"], skip_special_tokens=True)
                action = extract_last_webshop_action(decoded_so_far)
                if action is None:
                    rollout["done"] = True
                    rollout["finish_reason"] = "invalid_action"
                    rollout["error"] = "no_valid_webshop_action"
                    continue

                try:
                    step_resp = requests.post(
                        f"{base}/step",
                        json={"env_idx": int(rollout["env_idx"]), "action": action},
                        timeout=args.webshop_replay_timeout,
                    )
                    ...
                    available = {} if done else webshop_available_actions(base, int(rollout["env_idx"]), args.webshop_replay_timeout)
                    obs_text = format_webshop_observation_block(state, available, reward, done)
                    remaining = sampling_params.max_tokens - len(rollout["response"])
                    obs_budget = min(args.webshop_observation_token_budget, max(0, remaining))
                    obs_ids = encode_masked_webshop_observation(
                        tokenizer,
                        obs_text,
                        obs_budget,
                    )
                    rollout["response"].extend(obs_ids)
                    rollout["mask"].extend([0] * len(obs_ids))
                    rollout["num_calls"] += 1
                    rollout["last_reward"] = reward
                    rollout["next_action_prefix"] = webshop_action_prefix(available)
                    if done:
                        rollout["done"] = True
                        rollout["finish_reason"] = "stop"
                    elif len(rollout["response"]) >= sampling_params.max_tokens:
                        rollout["done"] = True
                        rollout["finish_reason"] = "length"
                    else:
                        rollout["finish_reason"] = step_finish_reason
                except Exception as exc:
                    rollout["done"] = True
                    rollout["finish_reason"] = "env_error"
                    rollout["error"] = f"{type(exc).__name__}: {str(exc)[:200]}"

        response_ids = [rollout["response"] for rollout in rollouts]
        finish_reasons = [rollout["finish_reason"] for rollout in rollouts]
        masks = [rollout["mask"] for rollout in rollouts]
        num_calls = [rollout["num_calls"] for rollout in rollouts]
        timeouts = [0] * len(rollouts)
        tool_errors = [rollout["error"] for rollout in rollouts]
        tool_outputs = [
            f"webshop_reward={rollout['last_reward']:.6f};done={bool(rollout['done'])}"
            if rollout["num_calls"] > 0
            else ""
            for rollout in rollouts
        ]
        tool_runtimes = [0] * len(rollouts)
        tool_calleds = [rollout["num_calls"] > 0 for rollout in rollouts]
        return (
            response_ids,
            finish_reasons,
            masks,
    ...
    for training_step in range(resume_training_step, num_training_steps + 1):
        ...
        if items is None:
            break
        if len(items) == 3:
            _, g_queries_list, g_ground_truths = items
        else:
            _, g_queries_list = items
            g_ground_truths = None

        with Timer("🔥 Generation time"):
            if args.webshop_interactive_rollout:
                if g_ground_truths is None:
                    raise ValueError("webshop_interactive_rollout requires ground truths in param_prompt_Q")
                response_ids, finish_reasons, masks, info = generate_webshop_rollouts_with_engines(
                    g_queries_list,
                    g_ground_truths,
                    generation_config,
        ...
        # Evaluate the model
        if eval_prompt_token_ids is not None and eval_freq > 0 and ((training_step - 1) % eval_freq == 0 or args.eval_at_step == training_step):
            if args.webshop_interactive_rollout:
                response_ids, finish_reasons, masks, info = generate_webshop_rollouts_with_engines(
...
def data_preparation_thread(
    ...
    for training_step in range(1, num_training_steps + 1):
        ...
        if args.webshop_rollout_trace_path:
            trace_path = Path(args.webshop_rollout_trace_path)
            ...
            with trace_path.open("a", encoding="utf-8") as f:
                for i in range(len(decoded_responses)):
                    row = {
                        ...
                        "index": i,
                        "query": decoded_queries[i] if decoded_queries is not None and i < len(decoded_queries) else "",
                        ...
                        "tool_output": str(tool_outputs[i]) if i < len(tool_outputs) else "",
        ...
        with Timer("📦 [Data Preparation Thread] Packing sequences"):
            skip_optimization = len(responses) == 0
...
def main(args: Args, tc: TokenizerConfig, model_config: ModelConfig, reward_fn: Callable):
    ...
    args.eval_freq = -1 if args.num_evals <= 0 else max(1, args.num_training_steps // args.num_evals)
    ...
    param_prompt_Q.put((None, queries_next, ground_truths_next))
    ...
    try:
        for training_step in range(resume_training_step, args.num_training_steps + 1):
            ...
            with Timer("[Main Thread] 📦 Getting packed sequences from thread"):
                ...
                if B == 0 or packed_data.get("skip_optimization", False):
                    print("[Main Thread] After filtering, there is no non-zero-gradient WebShop group. Skipping optimization for this step.")
                    skip_batch = True
            ...
            try:
                ...
                eval_reward_ground_truths = eval_ground_truths
                if args.critic_error_enabled:
                    eval_reward_ground_truths = disable_critic_error_pool_for_ground_truths(eval_ground_truths)

                eval_result = asyncio.run(
                    reward_fn(
                        eval_responses,
                        eval_decoded_responses,
                        eval_reward_ground_truths,
                        eval_dataset_names,
                        eval_finish_reasons,
                        eval_infos,
                        source_datasets=eval_original_dataset_names,
...
if __name__ == "__main__":
    ...
    async def reward_fn(
        ...
        if args.critic_error_enabled and is_training and training_step is not None:
            if args.critic_error_pool_path and (
                training_step == 1 or training_step % max(1, args.critic_error_update_every_steps) == 0
            ):
                with Timer("[Data Preparation Thread] Calculating rewards -- 🧭 Mining Critic Error weaknesses"):
                    critic_pool_stats = await mine_and_update_critic_error_pool(
                        decoded_responses=decoded_responses,
                        ground_truths=ground_truths,
                        training_step=training_step,
                        pool_path=args.critic_error_pool_path,
                        model_name=args.critic_error_judge_model,
                        base_url=args.critic_error_openai_base_url,
                        api_key_env=args.critic_error_api_key_env,
                        max_active=args.critic_error_max_active_weaknesses,
                    )
                    metrics["objective/critic_error_pool_added"] = critic_pool_stats.get("added", 0)
                    metrics["objective/critic_error_pool_updated"] = critic_pool_stats.get("updated", 0)
                    metrics["objective/critic_error_pool_active"] = critic_pool_stats.get("active", 0)

            if args.critic_error_pool_path and (
                training_step == 1 or training_step % max(1, args.critic_error_inject_every_steps) == 0
            ):
                with Timer("[Data Preparation Thread] Calculating rewards -- 🧩 Injecting Critic Error rubrics"):
                    ground_truths = inject_active_critic_rubrics_into_ground_truths(
                        ground_truths,
            ...
            with Timer("[Data Preparation Thread] Calculating rewards -- 🏆 Applying verifiable reward"):
                ...
                if args.critic_error_enabled and is_training and training_step is not None and args.critic_error_pool_path:
                    critic_retire_stats = update_critic_error_pool_from_logs(
                        ...
                        training_step=training_step,
                        trigger_threshold=args.critic_error_trigger_threshold,
                        retire_after_absent_batches=args.critic_error_retire_after_absent_batches,
                    )
                    metrics["objective/critic_error_pool_retired"] = critic_retire_stats.get("retired", 0)
                    metrics["objective/critic_error_pool_active_after_retire"] = critic_retire_stats.get("active", 0)
