...
LAUNCH="$RUN_DIR/launch_critic_v5_agentgym.sh"
PID_FILE="$LOG_DIR/critic_v5.pid"
LOG_FILE="$LOG_DIR/critic_v5.log"
...
while true; do
  ...
  if [ -f "$BASELINE_LOG" ] && grep -q "finished training" "$BASELINE_LOG"; then
